...
def register() :
	...
	query = "http://"+ip+":8080/api/clients/vehicleRegister/"+str(VLPN)+"/"+str(UVID)
			...
			if vehicleDetails[1] != str('null')  : # CRIMINAL RECORD
				...
				if len(availableSpots)<2  :
					...
					if Choice > i:
						...
						query = "http://192.168.178.44:8080/api/clients/choice/"+str(StartTime)+"/"+str(End)+"/"+str(availableSpots[Choice])+"/"+str(VehiclePlateNumber)
